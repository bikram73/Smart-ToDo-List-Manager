...
def check_reminders(authorization: str = Header(default=None)):
    """
    ...
    """
    # Security Check: Verify the request comes from Vercel
    # Add CRON_SECRET to your Vercel Environment Variables
    cron_secret = os.getenv("CRON_SECRET")
    if cron_secret:
        cron_secret = cron_secret.strip()  # Remove any accidental spaces from Vercel env var
        if authorization != f"Bearer {cron_secret}":
            print(f"❌ Cron Auth Failed. \n   Expected: 'Bearer {cron_secret}' \n   Received: '{authorization}'")
...
async def start_background_service():
    # Only run the background loop if NOT on Vercel (Vercel uses Cron Jobs)
    if not os.getenv("VERCEL"):
        print("💻 Local environment detected: Starting background reminder loop.")
        asyncio.create_task(run_reminder_loop())
    else:
        print("☁️ Vercel environment detected: Background loop disabled (relying on Cron Job).")
